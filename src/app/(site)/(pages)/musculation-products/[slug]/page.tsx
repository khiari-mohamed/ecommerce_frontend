...
export default async function MusculationProductPage(props: { params: { slug: string } }) {
  ...
  try {
    ...
    if (product && product._id) {
      ...
      if (reviews.length > 0) {
        averageRating = reviews.reduce((acc, r) => acc + parseInt(r.stars, 10), 0) / reviews.length;
      }
    }
  } catch (e) {
    product = null;
  ...
  // Helper to get the image src with fallback
  const getImageSrc = () => {
    if (product.cover && product.cover.trim() !== "") {
      return product.cover.startsWith("/")
        ? product.cover
        : "/" + product.cover;
    }
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1">
          ...
          <div className="mb-2 text-lg text-gray-700">
            ...
            {Number(product.prix).toLocaleString("fr-FR", { style: "currency", currency: "DZD" })}
            {product.promo && (
              <span className="ml-2 text-red-500 line-through">
                {Number(product.promo).toLocaleString("fr-FR", { style: "currency", currency: "DZD" })}
              </span>
            )}
          </div>
          <div className="mb-2">
          ...
          <div className="mb-2">
            ...
            {product.qte && Number(product.qte) > 0 ? "En stock" : "Rupture de stock"}
          </div>
          <div className="mt-6">
            <h2 className="text-xl font-semibold mb-2">Description</h2>
            <div
              className="text-gray-600"
              dangerouslySetInnerHTML={{ __html: product.description_fr || "Aucune description disponible." }}
            />
          </div>
        </div>
      </div>
