...
import { getBlogBySlug, Blog } from "@/services/blog.service";
...
const BlogDetails = ({ slug }: BlogDetailsProps) => {
  ...
  // Use description_cover for SEO description if present, else fallback to meta_description_fr or meta
  const description =
    blog?.description_cover || blog?.meta_description_fr || blog?.meta || "";

  // Use content or description as main content
  const content =
    blog?.content ||
    blog?.description ||
    `<p>Aucune description disponible pour cet article.</p>`;

  // Use content_seo for SEO quote if present
  const contentSeo =
    blog?.content_seo ||
    "‘‘Aucune citation SEO disponible pour cet article.’’";

  // Use author and author_role if present, else fallback to generic
  const author = blog?.author || "Auteur inconnu";
  const authorRole = blog?.author_role || "Expert Nutrition";

  // Use tags if present (assuming blog.tags: string[]), else fallback to empty array
  const tags: string[] = (blog as any)?.tags || [];

  return (
    <>
      <Breadcrumb title={title} pages={["blog details"]} />
            ...
            <>
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-10 mt-10">
                  ...
                  {/* Social Links (static, but you can make them dynamic if you have blog.socials) */}
