...
const TopPromotionSection = () => {
  ...
  const handleAddToCart = (promo: TopPromotion) => {
    ...
    dispatch(addItemToCart({ ...normalized, quantity: 1, image: normalized.cover }));
  };
    ...
    <section className="overflow-hidden">
      <div className="w-full mx-auto px-2 sm:px-4 pb-10 sm:pb-15 border-b border-gray-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-4 sm:gap-6 md:gap-7.5">
          {loading ? (
            <div className="col-span-1 sm:col-span-2 md:col-span-2 lg:col-span-4 xl:col-span-4 flex justify-center items-center py-10">
            ...
            promotions.slice(0, 4).map((promo, key) => {
              return (
                <div
                  key={promo._id || key}
                  className="group relative flex flex-col h-full bg-white rounded-md overflow-hidden border transition-all duration-300 hover:border-primary/20 hover:shadow-lg"
                >
                  <div className="relative flex items-center justify-center p-0 sm:p-4 aspect-square bg-gray-50">
                    {/* Discount badge */}
                    ...
                    <div className="absolute right-0 bottom-0 translate-x-full w-full flex flex-row flex-nowrap gap-2 p-5.5 ease-linear duration-300 group-hover:translate-x-0 overflow-x-auto z-20">
                      <button
                        ...
                        tabIndex={-1}
                        type="button"
                        onClick={e => { e.preventDefault(); handleQuickViewUpdate(promo); openModal(); }}
                      >
                        {/* Eye icon SVG */}
                      ...
                      <button
                        ...
                        tabIndex={-1}
                        type="button"
                        onClick={e => { e.preventDefault(); handleItemToWishList(promo); }}
                      >
                        {/* Heart icon SVG */}
                        <svg className="fill-current" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <div className="flex flex-row flex-nowrap items-center justify-center gap-2 mb-2 mt-2">
                    <div className="flex flex-row flex-nowrap items-center gap-1">
                      ...
                      <span className="text-custom-sm ml-1">
                        {`(${(promo.product?.reviews?.length && promo.product.reviews.length > 0)
                          ? promo.product.reviews.length
                          : (() => {
                              const str = String(promo.product?._id || promo.productId || promo._id);
                              let hash = 0;
                  ...
                  <div className="flex flex-row flex-nowrap items-center justify-center gap-2 mb-3 mt-2">
