...
const MyAccount = () => {
  ...
  // Centralized user info state
  const [userInfo, setUserInfo] = useState({
    firstName: "Service",
    lastName: "SOBITAS",
    name: "Service SOBITAS",
    email: "contact@protein.tn",
    phone: "+216 27 612 500 | +216 73 200 169",
    address: "Rue Ribat, 4000 Sousse Tunisie",
  });

  // Dummy orders array for demonstration; replace with real data as needed
  ...
  // Handler for updating user info from AddressModal
  const handleAddressModalSave = (updated) => {
    setUserInfo((prev) => ({ ...prev, ...updated }));
    setAddressModal(false);
  };

  // Handler for updating name from account details
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
              ...
              <div className="flex xl:flex-col">
                <div className="hidden lg:flex flex-wrap items-center gap-5 py-6 px-4 sm:px-7.5 xl:px-9 border-r xl:border-r-0 xl:border-b border-gray-3">
                  ...
                  <div>
                    ...
                    <p className="text-custom-xs">Membre depuis septembre 2010</p>
                  </div>
                </div>
          ...
          <!--== user dashboard content start ==--> */}
            ...
            >
              ...
              Depuis le tableau de bord de votre compte, vous pouvez consulter vos commandes récentes, gérer vos adresses de livraison et de facturation, et modifier votre mot de passe et les détails de votre compte.
              </p>
            </div>
              ...
              <div className="xl:max-w-[370px] w-full bg-white shadow-1 rounded-xl">
                ...
                <div className="p-4 sm:p-7.5">
                  <div className="flex flex-col gap-4">
                    {(() => {
                      // Use latest commande for the user
                      const latestCommande = (typeof userOrders !== 'undefined' && userOrders.length > 0) ? userOrders[0] : null;
                      const fallbackName = "Service SOBITAS";
                      const fallbackPhone = "+216 27 612 500 | +216 73 200 169";
                      const fallbackEmail = "contact@protein.tn";
                      const fallbackAddress = "Rue Ribat, 4000 Sousse Tunisie";
                      const fallbackDesc = "PROTEINE TUNISIE - SOBITAS : Votre Partenaire Nutrition & Équipement Sportif en Tunisie Experts depuis 2010 – Boostez vos performances avec des compléments certifiés (protéines , créatine, BCAA, oméga3 ) et du matériel de musculation professionnel ( équipements cardio fitness). Atteignez vos objectifs prise de masse, force, ou récupération avec des produits haute qualité, disponibles à Sousse et livrés en Tunisie.";

                      const shippingName = latestCommande?.livraison_nom || latestCommande?.nom || userInfo.name || fallbackName;
                      const shippingPrenom = latestCommande?.livraison_prenom || latestCommande?.prenom || '';
                      const shippingEmail = latestCommande?.livraison_email || latestCommande?.email || userInfo.email || fallbackEmail;
                      const shippingPhone = latestCommande?.livraison_phone || latestCommande?.phone || userInfo.phone || fallbackPhone;
                      const shippingAddress = [latestCommande?.livraison_adresse1, latestCommande?.livraison_adresse2]
                        .filter(Boolean).join(' ') || [latestCommande?.adresse1, latestCommande?.adresse2].filter(Boolean).join(' ') || userInfo.address || fallbackAddress;
                      const shippingCity = latestCommande?.livraison_ville || latestCommande?.ville || '';
                      const shippingRegion = latestCommande?.livraison_region || latestCommande?.region || '';
                      const shippingPostal = latestCommande?.livraison_code_postale || latestCommande?.code_postale || '';
                      return <>
                        <p className="flex items-center gap-2.5 text-custom-sm">
                          ...
                          Name: {shippingName} {shippingPrenom}
                        </p>
                        <p className="flex items-center gap-2.5 text-custom-sm">
                          <svg
                            className="fill-current"
                        ...
                        <p className="flex gap-2.5 text-custom-sm">
                          ...
                          Address: {[shippingAddress, shippingCity, shippingRegion, shippingPostal].filter(Boolean).join(', ') || 'N/A'}
                        </p>
                      </>;
                    })()}
                  </div>
                ...
                <div className="p-4 sm:p-7.5">
                  <div className="flex flex-col gap-4">
                    {(() => {
                      ...
                      const billingName = latestCommande?.nom || userInfo.name || fallbackName;
                      const billingPrenom = latestCommande?.prenom || '';
                      const billingEmail = latestCommande?.email || userInfo.email || fallbackEmail;
                      const billingPhone = latestCommande?.phone || userInfo.phone || fallbackPhone;
                      const billingAddress = [latestCommande?.adresse1, latestCommande?.adresse2]
                        .filter(Boolean).join(' ') || userInfo.address || fallbackAddress;
                      const billingCity = latestCommande?.ville || '';
                      const billingRegion = latestCommande?.region || '';
                      const billingPostal = latestCommande?.code_postale || '';
                      return <>
                        <p className="flex items-center gap-2.5 text-custom-sm">
                          <svg
                            className="fill-current"
                          ...
                          Name: {billingName} {billingPrenom}
                        </p>
                        <p className="flex items-center gap-2.5 text-custom-sm">
                          <svg
                            className="fill-current"
                      ...
                      Address: {[billingAddress, billingCity, billingRegion, billingPostal].filter(Boolean).join(', ') || 'N/A'}
                    </p>
                      </>;
                    })()}
                  </div>
          ...
          <!-- details tab content start --> */}
            ...
            >
              <form>
                ...
                Votre nom sera affiché ainsi dans la section Compte et dans les avis.
                </p>
